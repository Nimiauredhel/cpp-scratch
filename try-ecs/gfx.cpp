...
static const std::string texture_paths[TEXTURE_ID_COUNT] =
{
    ...
    "textures/wall.png",
    "textures/door.png",
    "textures/humanoid1.png",
    "textures/humanoid2.png",
};
...
static void gfx_draw_element(Transform &transform, Sprite &sprite, Vector2Int offset)
{
    if (sprite.texture_id == TEXTURE_NONE) return;

    SDL_Texture *texture_ptr = gfx_get_texture_ptr(sprite.texture_id);

    if (texture_ptr == nullptr)
    {
        std::cout << "Null texture pointer associated with valid texture id." << std::endl;
        return;
    }
    ...
    if (draw_pos.x < 0 || draw_pos.x >= main_window_data.width
        || draw_pos.y < 0 || draw_pos.y >= main_window_data.height) return;

    SDL_Rect destination = { draw_pos.x * main_window_data.tile_size, draw_pos.y * main_window_data.tile_size, 0, 0 }; 
    SDL_QueryTexture(texture_ptr, NULL, NULL, &destination.w, &destination.h);
    destination.w *= transform.scale.x;
    destination.h *= transform.scale.y;
...
static void gfx_present(void)
{
    ...
    Entity *entities = current_scene->GetEntities();
    Transform *transforms = current_scene->GetTransforms();
    Sprite *sprites = current_scene->GetSprites();
    ...
    {
        offset = { mid_screen.x-transforms[focal_entity].position.x, mid_screen.y-transforms[focal_entity].position.y};
    }
    ...
    {
        if (!(entities[i].signature & COM_FLAG_SPRITE)) continue;
        ...
        {
            gfx_draw_element(transforms[i], sprites[i], offset);
        }
    }

    if (focal_entity >= 0)
    {
        gfx_draw_element(transforms[focal_entity], sprites[focal_entity], offset);
    }
